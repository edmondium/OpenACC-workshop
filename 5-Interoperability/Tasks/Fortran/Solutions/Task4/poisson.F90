...
subroutine solveKSpace(rhoHat)
  ...
  integer :: ix, iy
  !$acc parallel present(rhoHat) private(kx, ky, ik2, ix, iy)
    !$acc loop
    do ix = 1, Nx
      !$acc loop
      do iy = 1, Ny
...
subroutine solveRSpace(rho, phi)
  ...
  integer :: plan, ierr
  integer :: stream
  !$acc data copyin(rho(1:Nx, 1:Ny)) copyout(phi(1:Nx, 1:Ny))
  ierr = cufftPlan2d(plan, Nx, Ny, CUFFT_Z2Z)           !! Build cuFFT plan [Note:CufftPlan]
  stream = acc_get_cuda_stream(acc_async_sync)          !! [Note:Stream]
  ierr = cufftSetStream(plan, stream)                   !! Move cuFFT to OpenACC stream
  !$acc host_data use_device(rho, phi)
  !! cuFFT needs device pointers
