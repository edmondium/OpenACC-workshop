...
void solveKSpace(cmplx*  restrict rhoHat) {
  #pragma acc parallel present(rhoHat)
  {
    #pragma acc loop
...
void normalise(cmplx* restrict phi) {
  ...
  #pragma acc parallel loop present(phi)
...
void solveRSpace(cmplx* restrict rho, cmplx* restrict phi) {
  #pragma acc data copyin(rho[0:Size]) copyout(phi[0:Size])
  {
    cufftHandle xform;
    CUFFTC(cufftPlan2d(&xform, Nx, Ny, CUFFT_Z2Z));       // Build cuFFT plan [Note:CufftPlan]
    cudaStream_t str =
      (cudaStream_t) acc_get_cuda_stream(acc_async_sync); // [Note:Stream]
    CUFFTC(cufftSetStream(xform, str));                   // Move cuFFT to OpenACC stream
    #pragma acc host_data use_device(rho, phi)            // cuFFT needs device pointers
    CUFFTC(cufftExecZ2Z(xform, rho, phi, CUFFT_FORWARD)); // FFT
    solveKSpace(phi);                                     // div grad ~ -k^2
    #pragma acc host_data use_device(phi)                 // As before
