...
do while ( (error > tol) .and. (iter < iter_max) )
  ...
  error = 0.d0
!$acc parallel loop reduction(max:error) copy(A(1:NM, 1:NM), Anew(1:NM, 1:NM))
  do j = 2, NM-1
    do i = 2, NN-1
...
! Swap input/output
!$acc parallel loop copy(A(1:NM, 1:NM), Anew(1:NM, 1:NM))
  do j = 2, NM-1
    do i = 2, NN-1
      A(i,j) = Anew(i,j)
    end do ! i
...
!$acc parallel loop copy(A(1:NM, 1:NM))
  do i = 2, NN-1
    A(i,1)  = A(i,NM-1)
    A(i,NM) = A(i,2)
  end do ! i
